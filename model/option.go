...
func UpdateOption(key string, value string) error {
	// Save to database first, and fail fast when DB write fails.
	option := Option{Key: key}
	err := DB.Where(Option{Key: key}).Assign(Option{Value: value}).FirstOrCreate(&option).Error
	if err != nil {
		return err
	}
	// Update OptionMap
...
func DeleteOption(key string) error {
	// Delete from database first; then clear runtime cache.
	err := DB.Where("`key` = ?", key).Delete(&Option{}).Error
	if err != nil {
		return err
	}
	return updateOptionMap(key, "")
}
